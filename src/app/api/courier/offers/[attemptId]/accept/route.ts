...
export async function POST(request: NextRequest, context: { params: Promise<{ attemptId: string }> }) {
  ...
  if (error || !data) return apiError("ORDER_UNAVAILABLE", error?.message === "COURIER_HAS_ACTIVE_ORDER" ? "Terminá o resolvé tu pedido activo antes de tomar otro." : "El pedido ya fue tomado por otro cadete.", 409);
