...
const schema = z.object({
  status: z.enum(["confirmed", "assigned", "heading_to_pickup", "at_pickup", "picked_up", "heading_to_delivery", "at_delivery", "delivered", "cancelled", "rejected", "incident"]),
  reason: z.string().trim().max(500).optional(),
  pickupConfirmed: z.boolean().optional(),
}).superRefine((value, context) => {
  if (value.status === "picked_up" && !value.pickupConfirmed) {
...
export async function POST(request: NextRequest, context: RouteContext<"/api/orders/[id]/status">) {
  ...
  const { data, error } = await sessionClient.rpc("transition_order_status", {
    order_id: id,
    target_status: parsed.data.status,
    reason_text: parsed.data.reason ?? null,
  });
  if (error || !data) return apiError("STATUS_CHANGE_FAILED", "No fue posible actualizar el pedido.", error?.message === "FORBIDDEN" ? 403 : 422);
