...
export async function POST(request: NextRequest) {
  ...
  try { const estimate = await estimateOrder(parsed.data); return NextResponse.json({ distanceMeters: estimate.distanceMeters, durationSeconds: estimate.durationSeconds, encodedPolyline: estimate.encodedPolyline, price: estimate.price, priceStatus: "confirmed" }); }
