...
export default async function CourierOrders() {
  ...
  return <><SiteHeader/><main className="mx-auto max-w-6xl px-4 py-6 sm:py-8"><header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-bold text-brand">OPERACIÓN</p><h1 className="mt-2 text-3xl font-bold">Pedidos y ofertas</h1><p className="mt-2 text-zinc-400">Elegí ofertas cercanas sin exponer datos de clientes hasta aceptarlas.</p></div><Link href="/courier" className="w-fit text-sm font-bold text-brand">← Volver al panel</Link></header>
    <section className="mt-8"><div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between"><h2 className="text-xl font-bold">Disponibles cerca tuyo</h2><span className="text-sm text-zinc-400">Ordenadas por distancia al retiro</span></div><NearbyOffers/></section>
