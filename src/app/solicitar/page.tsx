...
export default async function RequestPage() {
  ...
  return <><SiteHeader /><main className="request-page mx-auto max-w-3xl px-4 py-10"><style>{`.request-page:has(> section) .request-intro { display: none; }`}</style><div className="request-intro"><p className="font-bold text-yellow-400">NUEVO PEDIDO</p><h1 className="mt-2 text-3xl font-bold">Solicita tu envio</h1><p className="mt-2 text-zinc-400">El remitente sera el titular de la cuenta. Solo necesitas cargar los datos de quien recibe.</p></div><OrderPushGate><OrderWizard senderName={profile.full_name} senderEmail={user.email ?? ""} /></OrderPushGate></main></>;
}
