...
import { orderDraftSchema, personNameSchema, phoneSchema } from "./schema";
...
describe("order validation", () => {
  ...
  it("requires funds for purchases and a valid slot for scheduled deliveries", () => {
    expect(orderDraftSchema.safeParse({ ...validOrder, serviceType: "purchase", product: { description: "Medicamentos", fragile: false, requiresPurchaseFunds: false, requiresReceipt: true } }).success).toBe(false);
