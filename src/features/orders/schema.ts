...
const productSchema = z.object({ description: z.string().max(500).optional(), size: packageSizeSchema.optional(), weightKg: z.coerce.number().positive("El peso debe ser mayor a 0").max(15, "El límite inicial es 15 kg").optional(), fragile: z.boolean().default(false), declaredValue: z.coerce.number().min(0).max(500000).optional(), requiresPurchaseFunds: z.boolean().default(false), purchaseFunds: z.coerce.number().positive().max(150000).optional(), requiresReceipt: z.boolean().default(false), notes: z.string().max(1000).optional() });
export const orderDraftSchema = z.object({
  serviceType: serviceTypeSchema, product: productSchema, pickup: addressSchema, delivery: addressSchema,
  senderName: personNameSchema, senderEmail: emailSchema, senderPhone: phoneSchema, recipientName: personNameSchema, recipientPhone: phoneSchema,
